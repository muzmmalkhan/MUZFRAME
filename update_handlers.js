...
code = code.replace(/const handleToggleSong = async.*?window\.open\([^)]+\);\s*\}\s*\};/s, '/* TOGGLE_SONG_PLACEHOLDER */');
code = code.replace(/const handleAddCustomSong = async.*?window\.open\([^)]+\);\s*\}\s*\};/s, '/* ADD_CUSTOM_SONG_PLACEHOLDER */');

code = code.replace(/\/\* TOGGLE_SONG_PLACEHOLDER \*\/\s*\/\* ADD_CUSTOM_SONG_PLACEHOLDER \*\//s, newHandlers);
